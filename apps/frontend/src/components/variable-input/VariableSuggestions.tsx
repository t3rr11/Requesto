...
export const VariableSuggestions = forwardRef<HTMLDivElement, VariableSuggestionsProps>(
  ({ show, variables, currentVariable, onInsert }, ref) => {
    ...
    const filteredSuggestions = variables.filter(v =>
      v.key.toLowerCase().includes(currentVariable.toLowerCase()),
    );
    ...
    return (
      <div
        ref={ref}
        className="absolute z-50 mt-1 w-full max-w-md bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-60 overflow-y-auto"
      >
        <div className="p-2">
